...
var songs,nowmusic= 0,debug=false;
$(document).ready(function(){
    ...
    Player.playnext();//先这样吧
    console.info("少年填坑中……好想快点把这个坑填好啊！！！");
...
Player = function(){
    this.player=document.getElementById("domplayer");
    this.state="init";
    ...
    this.progressDo = function(){
        that.player.addEventListener("timeupdate",function(){
            var position = (that.player.currentTime/that.player.duration)*(parseInt($("aside").css("height"))-parseInt($(that.progress).css("height")))+"px";
            TweenMax.to(that.progress, .5, {top:position,ease: Linear.easeNone});
            var m = parseInt(that.player.currentTime/60);
            var s = parseInt(that.player.currentTime%60);
            //$(that.progress).children("span:nth-child(3)").children("p").children("small").html(m+":"+s);
        },false);
    };
    this.playnext = function(){
        nowmusic=random(0,songs.length);
        this.getmedia();
        this.newplay();
        $(".suki").removeClass("sukidesu");//��ʱ��Ϊ�и�ֱ������Ϊ��ϲ��

    };
    this.getlrc = function(){
        if(songs[nowmusic].src.lrc){
            $.ajax({url:"getlrc.php",async:true,dataType:"text",data:{href:songs[nowmusic].src.lrc[0].href},success: function(data){
                lrc.parseLyric(data);
            }
            })
        }
    ...
    this.getbackground = function(){
        var BGI = document.body;
        if(songs[nowmusic].src.background){
            BGI.style.backgroundImage = "url('" + songs[nowmusic].src.background[0].href +"?imageMogr2/thumbnail/" + document.body.clientWidth + "x')";
        } else if(songs[nowmusic].src.cover) {
            BGI.style.backgroundImage = "url('" + songs[nowmusic].src.cover[0].href +"?imageMogr2/thumbnail/" + document.body.clientWidth + "x')";
        }else {
            if(BGI.style.backgroundImage=="url(img/background.png)"){

            } else {
                BGI.style.backgroundImage ="url(img/background.png)";
            }
        }
    };
    this.getcover = function(){
        var COVER = document.getElementById("cover");
        if(songs[nowmusic].src.cover){
            COVER.src = songs[nowmusic].src.cover[0].href + "?imageView2/1/w/200/h/200/interlace/1";
        } else {
            if(COVER.src=="img/default.jpg"){
    ...
    this.newplay = function(){
        document.title=songs[nowmusic].title;
        $(".information h3").html(songs[nowmusic].title);
        $(".information p").html(songs[nowmusic].singer);
        lrc.removeAllChild();
        lrc.lrc.innerHTML="<li style='line-height:1000%;'>♪~ ゛(´д｀*)゛~♪~♪　ﾌﾝﾌﾝ</li>";
        if(songs[nowmusic].src){
            Playbutton.pause();
            this.player.src=songs[nowmusic].src.src[0].href;
            this.getlrc();
            this.getbackground();
            this.getcover();
            if(songs[nowmusic].src.like) $(".suki").addClass("sukidesu");
            if(songs[nowmusic].src.dislike) {
                this.playnext();
                return ;
            }
            } else {
            $.ajax({
                ...
                dataType:"json",
                data:{
                    song:JSON.stringify(songs[nowmusic])
                },
                success:function(data){
                    if(data.src){
                        for(i=0;i<data.src.length;i++){
                            data.src[i] = JSON.parse(data.src[i]);
                        }
                    }
                    if(data.cover){
                        for(i=0;i<data.cover.length;i++){
                            data.cover[i] = JSON.parse(data.cover[i]);
                        }
                    }
                    if(data.lrc){
                        for(i=0;i<data.lrc.length;i++){
                            data.lrc[i] = JSON.parse(data.lrc[i]);
                        }
                    }
                    if(data.background){
                        for(i=0;i<data.background.length;i++){
                            data.background[i] = JSON.parse(data.background[i]);
                        }
                    }
                    songs[nowmusic].src=data;
                    that.newplay();
        ...
        $(this)
            ...
            .on(opts.event, function(ev) {
                ...
                $ink
                    ...
                    .css({height: size/2, width: size/2});
